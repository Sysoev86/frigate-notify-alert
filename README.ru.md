...
**В первую очередь это лёгкая сигнализация.** События уходят в Telegram как можно
быстрее — в реальном времени по MQTT (push, а не опрос по расписанию), чтобы ты успел
среагировать на несанкционированный доступ. Нужен звук вместо беззвучного режима —
поставь группе `silent: False`; мешает собственный проход по объекту — поставь на паузу
кнопкой прямо в чате.
...
## Возможности
- 🚨 **Работает как сигнализация:** события в реальном времени (MQTT push) — чтобы среагировать на несанкционированный доступ; со звуком по `silent: False`.
- ⏰ **Мгновенный текст после простоя** (опция `IDLE_ALERT_AFTER`, по умолчанию выкл.): если событий не было, скажем, час — первое приходит сразу текстом со звуком, ещё до фото/видео.
...
| Что | Зачем | Без этого |
|---|---|---|
| **MQTT** | события в реальном времени (`frigate/events`) | события только через опрос API — с задержкой |
| **Snapshots** | даёт фото события (`has_snapshot`) | без фото событие вообще не отправится |
| **Record (записи)** | даёт видео-клип (`has_clip`) | без видео событие вообще не отправится |
...
Заметки:
- `snapshots`, `record` и `objects.track` работают **глобально или на камеру** — важно
  итоговое значение у камеры.
- **`mode: active_objects` не случайно:** при `mode: motion` маска движения над зоной,
  где ходят объекты, молча убивает клипы (фото приходит, видео — нет) — см.
  [Диагностику](#диагностика).
- Пример для Frigate **0.14+** (проверено на 0.17); в 0.13 хранение было в
  `record.events.retain`. Документация:
...
sudo ./manage.sh setup             # зависимости + systemd-юниты (по группам) + запуск
```
Ручной запуск без systemd: `./manage.sh run [группа]`. Версия: `./manage.sh version`.
...
GROUPS = {
    ...
    "group2": {
        ...
        "cameras": ["vhod"],
        "silent": False,              # необязательно: громкие уведомления (по умолч. True = тихо)
        "name": "Вход",
    },
}
...
| `silent` | нет | `True` (по умолчанию) → сообщения приходят беззвучно. `False` → полноценные громкие уведомления. |
| `startup_message` | нет | `True` (по умолчанию) → при каждом старте одно тихое сообщение «мониторинг запущен» (встроенная проверка связки: камеры, объекты, зоны, версия). `False` → выключить. |
...
| Параметр | Описание |
|---|---|
| `FRIGATE_URL` | Адрес API Frigate, откуда качаются фото/видео. Нужен **внутренний порт 5000 (без аутентификации)** — порт 8971 не подойдёт (бот не шлёт логин/пароль). |
...
| `MAX_CLIP_MB` | `45` | Лимит размера клипа. Боты Telegram не могут загрузить файл больше 50 МБ, а гигантские клипы (часовые события) убивали процесс по памяти; такие события приходят обычным альбомом с **обрезанным клипом (первые N секунд) и пометкой**. |
...
**панель, зависящую от состояния**. Когда уведомления включены — внизу только кнопки
паузы (`⏸ 15 мин / 1 час / 3 часа / до утра`); на паузе панель показывает «🔕 пауза
до X» (закреплена) и добавляет кнопку `▶️ Включить` + кнопки продления. Когда пауза
истекает, панель сама возвращается в обычный вид в течение минуты. Пауза действует
только на ту группу, в чьём чате нажата кнопка, и переживает перезапуск.
...
трогают systemd (`setup`/`install`/`start`/`stop`/`restart`/`enable`/`disable`/`update`),
требуют `sudo`.

| Команда | Что делает | Когда нужна |
|---|---|---|
| `setup` | Первая установка целиком: зависимости + systemd-юниты + запуск. Единственная команда для свежей установки. | Первый раз. |
| `run [группа]` | Ручной запуск без systemd (если группа одна — подставится сама). | Отладка/проба. |
| `doctor` | Самодиагностика всей цепочки: конфиг, Frigate API, имена камер/зон (с подсказками при опечатках), snapshots/record по камерам, медиа у свежих событий, MQTT, токен и доступ к чатам, статус сервисов. | Когда «ничего не приходит». |
| `install` | (Пере)ставит systemd-юниты (по группам из `config.py`), включает автозапуск. | После добавления новой группы. |
...
## Как это работает
Скрипт получает события в реальном времени из MQTT-топика `frigate/events` (плюс опрос
`/api/events` как подстраховка), фильтрует по камере, объекту и (опционально) зоне,
дожидается готовности snapshot + clip и шлёт их медиа-группой. Доставка — как можно
быстрее: событие уходит push'ом по MQTT, опрос API — лишь подстраховка, если MQTT
недоступен. Поэтому скрипт годится как лёгкая сигнализация: поставь `silent: False`
для звукового оповещения. При старте события, завершившиеся до запуска, игнорируются —
истории в чате не будет.
...
## Диагностика
- **Начни с `./manage.sh doctor`** — проверяет всю цепочку (конфиг, Frigate API,
  имена камер/зон, медиа у свежих событий, MQTT, Telegram) и показывает, где обрыв.
